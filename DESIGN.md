...
The audio callback owns no file, network, logging, lock, or heap allocation work in steady state. `BrickMawLimiter::prepare` allocates fixed delay rings; `processBlock` uses bounded loops capped by channel count and the prepared lookahead window. Non-finite samples are sanitized, and the final output is clamped to the configured digital sample ceiling.
